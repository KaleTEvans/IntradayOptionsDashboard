...
export async function fetchTradeData(ticker, right, strikes, quantity, cost) {
    try {
        const response = await fetch(`${py_server_path}/option-data/${ticker}/option/time-and-sales/all?right=${right}&strikes=${strikes}&quantity=${quantity}&cost=${cost}&limit=1000`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
